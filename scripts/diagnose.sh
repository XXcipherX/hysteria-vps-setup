...
check_files() {
  ...
  if [[ -f "$CADDY_CONFIG" ]]; then
    status_line OK "Caddy config exists"
  else
    status_line FAIL "Caddy config missing: $CADDY_CONFIG"
  fi
  if [[ -f "$HYSTERIA_CONFIG" ]]; then
    local mode
...
check_docker() {
  ...
  for container in hysteria hysteria-masquerade; do
    if container_running "$container"; then
      status_line OK "container $container is running"
    else
      status_line FAIL "container $container is not running"
    fi
  done
...
PY
  ...
  if [[ -f "$CADDY_CONFIG" ]] && have_cmd docker \
    && docker image inspect caddy:2-alpine >/dev/null 2>&1; then
    if docker run --rm -v "$CADDY_CONFIG:/etc/caddy/Caddyfile:ro" \
      caddy:2-alpine caddy validate --config /etc/caddy/Caddyfile >/dev/null 2>&1; then
      status_line OK "Caddy image validates the Caddyfile"
    else
      status_line FAIL "Caddy image rejected the Caddyfile"
    fi
  else
    status_line INFO "Caddy image is unavailable; skipped Caddy validation"
  fi
}
...
check_ports() {
  ...
  if ! port_listening tcp 4123; then
    status_line FAIL "4123/tcp is not listening"
  elif port_has_non_loopback_listener tcp 4123; then
    status_line FAIL "4123/tcp is public; expected loopback-only"
  else
    status_line OK "4123/tcp is loopback-only"
  fi
...
emit_json() {
  ...
  printf '"caddy_config":%s,' "$(bool_command test -f "$CADDY_CONFIG")"
  printf '"hysteria_running":%s,' "$(bool_command container_running hysteria)"
  printf '"masquerade_running":%s,' "$(bool_command container_running hysteria-masquerade)"
  ...
  printf '"tcp_4123_loopback":%s,' "$(if port_listening tcp 4123 && ! port_has_non_loopback_listener tcp 4123; then printf true; else printf false; fi)"
